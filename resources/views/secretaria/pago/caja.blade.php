 @extends('layouts.Secretaria',['title'=>'Secretaria | Caja','headertitle'=>'Caja','viewtitle'=>'Todos','page'=>'Caja'])
...
<script src="{{ asset('assets/js/jquery-ui.min.js')}}"></script>
  ...
  <script type="text/javascript">
	...
	jQuery(function($) {
					...
					@component('components.js.select-search',['name'=>'#alumno','ruta'=>route('Secretaria.Caja.Alumno.Search')])
	...
	$.ajax({
	url:"{!! route('Secretaria.Caja.Showtable') !!}",
	type:'POST',
	data:formData,
...
$('#mystep2').html(msg.tabla);
	...
	complete : function(xhr, status) {
				...
				var formData = new FormData($("#formcreate")[0]);
	$.ajax({
	url:"{!! route('Secretaria.Caja.Store') !!}",
	type:'POST',
	data:formData,
...
messageSucess(msg);
	...
	"sAjaxSource": "{!!route('Secretaria.Caja.Retrieve')!!}"	,
 "aaSorting": [],
 "language": {
	...
	var formData = new FormData($("#formcreate")[0]);
	$.ajax({
	url:"{!! route('Secretaria.Caja.Create') !!}",
	type:'POST',
	data:formData,
...
messageError(msg);
	...
	function deudas(alumno) {
		...
		@component('components.js.ajax-submit')
		    ...
		    @slot('rutaAjax')
				"{!! route('Secretaria.Deuda.Alumno.Duedas') !!}"
			@endslot
