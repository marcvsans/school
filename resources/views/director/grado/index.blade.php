@extends('layouts.director',['title'=>'Director | Grado','headertitle'=>'Grado','viewtitle'=>'Todos','page'=>' Grado'])
...
<div id="modal-registro" class="modal">
	<div class="modal-dialog" >
		...
		'formId'=>'form-create',
		'id'=>'widget',
		'title'=>'Formulario de Registro de Grados'])
		 	 	  @slot('toolbar')
	  <a href="#" >
	  ...
	  @slot('body')
				<input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
							...
							<label class="control-label col-xs-12 col-sm-3 no-padding-right" for="state">Nombre</label>

							<div class="col-xs-12 col-sm-6">
								<select id="nivel" name="numero" class="select2" data-placeholder="Nombre"  >
									<option value=""></option>				
									@foreach ($nombres as $key => $nombre)
										   <option value="{{$key + 1}}" >{{$nombre}} </option>
									@endforeach  
								</select>
							</div>
...
$('#menu-grado').addClass('active');				
	...
	@component('components.js.table',['route'=>route('Director.Grado.Retrieve')])
...
@component('components.js.validate-form')
      ...
      @slot('rules')
		  ...
		  nivel:{required:true } ,
		  numero:{required:true,range:[1,6]}
	@endslot
