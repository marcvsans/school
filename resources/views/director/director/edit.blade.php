@extends('layouts.director',['title'=>'Director | Director','headertitle'=>'Director','viewtitle'=>$Persona->nombres ." ". $Persona->apellidos,'page'=>'Director'])
				...
				<div class="col-lg-2 col-md-4 col-sm-6 col-xs-12 ">
					<div class="clearfix">
						<select class="select2  col-xs-4 col-sm-4"   data-placeholder="Seleccione" name="estado" >
							<option value=''></option>	
							@foreach ($director->estados as $estado)
								<option value="{{$estado}}" @if($estado==$director->estado) selected="" @endif>{{$estado}} </option>
							@endforeach  
						</select>
					</div>
				</div>
...
@section('script')
  ...
  <script type="text/javascript">
	...
	$('#director-edit').addClass('active').removeClass('hide');   
      ...
      @slot('rules')
       ...
       @slot('valid')
		...
		@component('components.js.ajax-submit')
		    ...
		    @slot('rutaAjax')
				'{{ route("Director.Director.Update",["alumno"=>"$Persona->nrodocumento"]) }}'
			@endslot
	        @slot('data')
				formData
			@endslot
